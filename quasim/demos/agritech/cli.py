...
from quasim.demos.agritech.kernels.simulation import run_simulation
...
def cmd_plan(args):
    ...
    print(f"Running agritech planning with seed={args.seed}")
    ...
    metrics = {
        k: results[k]
        for k in ["yield_kg_ha", "water_use_eff", "risk_of_loss", "profit_margin"]
        if k in results
    }
    ...
    output_dir = Path("artifacts/agritech") / f"run_{args.seed}"
...
def cmd_simulate(args):
    ...
    print(f"Running agritech simulation with seed={args.seed}")
    ...
    if capture:
        output_dir = Path("artifacts/agritech") / f"sim_{args.seed}"
        output_dir.mkdir(parents=True, exist_ok=True)
