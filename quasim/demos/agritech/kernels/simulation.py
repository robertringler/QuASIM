"""Simulation kernel for agritech."""
...
def run_simulation(
    ...
    # Add KPIs
    results["yield_kg_ha"] = float(np.abs(rng.randn() * 10) + 0)
    results["water_use_eff"] = float(np.abs(rng.randn() * 10) + 5)
    results["risk_of_loss"] = float(np.abs(rng.randn() * 10) + 10)
    results["profit_margin"] = float(np.abs(rng.randn() * 10) + 15)
