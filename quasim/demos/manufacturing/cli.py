...
from quasim.demos.manufacturing.kernels.simulation import run_simulation
...
def cmd_plan(args):
    ...
    print(f"Running manufacturing planning with seed={args.seed}")
    ...
    metrics = {
        k: results[k]
        for k in ["MTBF", "downtime_pct", "throughput_units_hr", "false_alarm_rate"]
        if k in results
    }
    ...
    output_dir = Path("artifacts/manufacturing") / f"run_{args.seed}"
...
def cmd_simulate(args):
    ...
    print(f"Running manufacturing simulation with seed={args.seed}")
    ...
    if capture:
        output_dir = Path("artifacts/manufacturing") / f"sim_{args.seed}"
        output_dir.mkdir(parents=True, exist_ok=True)
