...
def run_simulation(
    ...
    # Add KPIs
    results["LMP_cost"] = float(np.abs(rng.randn() * 10) + 0)
    results["curtailment_pct"] = float(np.abs(rng.randn() * 10) + 5)
    results["reserve_viols"] = float(np.abs(rng.randn() * 10) + 10)
    results["CO2_tonnes"] = float(np.abs(rng.randn() * 10) + 15)
