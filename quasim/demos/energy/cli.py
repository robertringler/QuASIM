...
from quasim.demos.energy.kernels.simulation import run_simulation
...
def cmd_plan(args):
    ...
    print(f"Running energy planning with seed={args.seed}")
    ...
    metrics = {
        k: results[k]
        for k in ["LMP_cost", "curtailment_pct", "reserve_viols", "CO2_tonnes"]
        if k in results
    }
    ...
    output_dir = Path("artifacts/energy") / f"run_{args.seed}"
...
def cmd_simulate(args):
    ...
    print(f"Running energy simulation with seed={args.seed}")
    ...
    if capture:
        output_dir = Path("artifacts/energy") / f"sim_{args.seed}"
        output_dir.mkdir(parents=True, exist_ok=True)
