...
def run_simulation(
    ...
    # Add KPIs
    results["VaR_99"] = float(np.abs(rng.randn() * 10) + 0)
    results["ES_97_5"] = float(np.abs(rng.randn() * 10) + 5)
    results["drawdown_max"] = float(np.abs(rng.randn() * 10) + 10)
    results["pnl_cvar_gap"] = float(np.abs(rng.randn() * 10) + 15)
