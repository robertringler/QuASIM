...
from quasim.demos.healthcare.kernels.simulation import run_simulation
...
def cmd_plan(args):
    ...
    metrics = {
        k: results[k]
        for k in ["power", "FPR", "responders_gain", "allocation_entropy"]
        if k in results
    }
    ...
    output_dir = Path("artifacts/healthcare") / f"run_{args.seed}"
