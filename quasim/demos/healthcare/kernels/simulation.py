"""Simulation kernel for healthcare."""
...
def run_simulation(
    ...
    # Add KPIs
    results["power"] = float(np.abs(rng.randn() * 10) + 0)
    results["FPR"] = float(np.abs(rng.randn() * 10) + 5)
    results["responders_gain"] = float(np.abs(rng.randn() * 10) + 10)
    results["allocation_entropy"] = float(np.abs(rng.randn() * 10) + 15)
