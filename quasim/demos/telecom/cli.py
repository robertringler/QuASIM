...
from quasim.demos.telecom.kernels.simulation import run_simulation
...
def cmd_plan(args):
    ...
    print(f"Running telecom planning with seed={args.seed}")
    ...
    metrics = {
        k: results[k]
        for k in ["SLA_viol_rate", "power_kWh", "forecast_MAE", "placement_cost"]
        if k in results
    }
    ...
    output_dir = Path("artifacts/telecom") / f"run_{args.seed}"
...
def cmd_simulate(args):
    ...
    print(f"Running telecom simulation with seed={args.seed}")
    ...
    if capture:
        output_dir = Path("artifacts/telecom") / f"sim_{args.seed}"
        output_dir.mkdir(parents=True, exist_ok=True)
