...
def run_simulation(
    ...
    # Add KPIs
    results["SLA_viol_rate"] = float(np.abs(rng.randn() * 10) + 0)
    results["power_kWh"] = float(np.abs(rng.randn() * 10) + 5)
    results["forecast_MAE"] = float(np.abs(rng.randn() * 10) + 10)
    results["placement_cost"] = float(np.abs(rng.randn() * 10) + 15)
