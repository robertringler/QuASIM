...
def test_governance_vm_scoring():
    vm = GovernanceVM()
    vm.register_rule(GovernanceRule(name="safety", condition_key="safety", threshold=0.5, weight=2.0))
    vm.register_rule(GovernanceRule(name="compliance", condition_key="compliance", threshold=1.0, weight=1.0))
