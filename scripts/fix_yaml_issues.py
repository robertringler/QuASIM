    ...
    try:
        ...
        try:
            ...
            if '{{' in content or '{%' in content:
                print(f"  -> Appears to be a template file, skipping")
                return True
            return False
    except Exception as e:
        print(f"Error processing {filepath}: {e}")
    ...
    for file_path in problematic_files:
        ...
        if full_path.exists():
            ...
            if first_line.startswith('{{-') or '{{' in first_line:
                print(f"  -> Helm template file (expected to fail standard YAML parsing)")
            elif '---' in open(full_path).read().count('---') > 1:
                print(f"  -> Multi-document YAML (valid format)")
            else:
